(function ($, window, MapSVG) {
    var MapSVGAdminRegionsListController = function (container, admin, mapsvg) {
        ...
        if (_this.database.getSchema()) {
            _this.database.getSchema().events.on("changed", function () {
    ...
    MapSVGAdminRegionsListController.prototype.getTemplateData = function () {
        var _this = this;
        var regions = _this.database.getLoadedAsArray();
        var regionOpts = _this.mapsvg.options.regions;
        regions.forEach(function (r) {
            r.fill =
                regionOpts && regionOpts[r.id] && regionOpts[r.id].fill
                    ? regionOpts[r.id].fill
                    : null;
        });
        return {
    ...
    MapSVGAdminRegionsListController.prototype.updateDataRow = function (region, row) {
        var _this = this;
        region.id_no_spaces = region.id.split(" ").join("_");
        ...
        d.id = region.id;
        d.id_no_spaces = region.id_no_spaces;
        d.fill =
            regionOpts && regionOpts[d.id] && regionOpts[d.id].fill ? regionOpts[d.id].fill : null;

        var newRow = $(_this.templates.item(d));

        row =
            row ||
            $("#mapsvg-region-" + region.id_no_spaces.replace(/(:|\(|\)|\.|\[|\]|,|=|@)/g, "\\$1"));
    ...
    MapSVGAdminRegionsListController.prototype.editRegion = function (region, scrollTo) {
        ...
        if (!region.selected) _this.mapsvg.selectRegion(region);
        ...
        _this.formBuilder = this.mapsvg.createForm({
