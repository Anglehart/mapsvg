(function ($, window, MapSVG) {
    ...
    MapSVGAdminSettingsController.prototype.viewLoaded = function () {
        ...
        _this.mapsvg.regionsRepository.getSchema().events.on("changed", function () {
            _this.updateGaugeFields();
        });
    ...
    MapSVGAdminSettingsController.prototype.setEventHandlers = function () {
        ...
        $("#mapsvg-controls-set-viewbox").on("click", function (e) {
            ...
            $("#mapsvg-controls-viewbox").val(v.toString()).trigger("change");
        });
        $("#mapsvg-controls-reset-viewbox").on("click", function (e) {
            e.preventDefault();
            var v = _this.mapsvg.svgDefault.viewBox;
            $("#mapsvg-controls-viewbox").val(v.toString()).trigger("change");
            _this.mapsvg.viewBoxReset(true);
        });
        ...
        $("#mapsvg-controls-zoom").on("change", ":radio", function () {
            ...
            on
                ? $("#mapsvg-controls-zoom-options").show()
                : $("#mapsvg-controls-zoom-options").hide();
        ...
        $("#mapsvg-controls-scroll").on("change", ":radio", function () {
            ...
            on
                ? $("#mapsvg-controls-scroll-options").show()
                : $("#mapsvg-controls-scroll-options").hide();
            _this.admin.updateScroll();
        });
        ...
        function change_file(path) {
            $.get(path, function (xmlData) {
                ...
                if (_data.svgDefault.width && _data.svgDefault.height) {
                    ...
                    _data.svgDefault.height = parseFloat(
                        _data.svgDefault.height.replace(/px/g, "")
                    );
                    ...
                    alert(
                        "MapSVG needs width/height or viewBox parameter to be present in SVG file."
                    );
                    return false;
                }
                _this.mapsvg.update({
        ...
        this.view.on("click", "#mapsvg-controls-file-remove", function (e) {
            change_file(MapSVG.urls.maps + "geo-calibrated/empty.svg");
        });
    ...
    MapSVGAdminSettingsController.prototype.setWidthViewbox = function () {
        ...
        if ($("#mapsvg-controls-ratio").is(":checked"))
            var k =
                _this.mapsvg.getData().svgDefault.width / _this.mapsvg.getData().svgDefault.height;
    ...
    MapSVGAdminSettingsController.prototype.setHeightViewbox = function () {
        ...
        if ($("#mapsvg-controls-ratio").is(":checked"))
            var k =
                _this.mapsvg.getData().svgDefault.height / _this.mapsvg.getData().svgDefault.width;
    ...
    MapSVGAdminSettingsController.prototype.updateGaugeFields = function () {
        ...
        fields.forEach(function (f) {
            select.append(
                "<option " +
                    (choroplethField == f.name ? "selected" : "") +
                    ">" +
                    f.name +
                    "</option>"
            );
