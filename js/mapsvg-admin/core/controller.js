(function ($, window, MapSVG) {
    var MapSVGAdminController = function (container, admin, mapsvg, events) {
        ...
        this.mapsvg = mapsvg;
        this.templates = {};
        this.templatesURL = this.templatesURL || MapSVG.urls.root + "js/mapsvg-admin/modules/";
                        ...
                        if (name) {
                            ...
                            if ($(tmpl).data("partial")) {
                                Handlebars.registerPartial(
                                    _this.nameCamel() + MapSVG.ucfirst(name) + "Partial",
                                    $(tmpl).html()
                                );
                            }
                        }
    ...
    MapSVGAdminController.prototype.setEventHandlersCommon = function () {
        ...
        _this.view
            ...
            .on("change", ".mapsvg-toggle-visibility", function () {
                ...
                var on = $(this).is(":checkbox")
                    ? MapSVG.parseBoolean($(this).prop("checked"))
                    : true;
                var selector = $(this).data("toggle-visibility");
            ...
            .on("click", ".btn-group-checkbox a", function () {
                ...
                setTimeout(function () {
                    ...
                    if (on)
                        btn.closest(".btn-group-checkbox")
            ...
            .on("click", ".mapsvg-tab-link", function (e) {
                e.preventDefault();
            ...
            if (validate == "function") {
                ...
                if (!MapSVG.isValidURL(val))
                    return new TypeError(
                        'MapSVG error: wrong URL format. URL must start with "http://"'
                    );
                ...
                if (data.substr(0, 1) == "[" || data.substr(0, 1) == "{") {
                    try {
                        ...
                        return new TypeError(
                            "MapSVG error: wrong object format for " + jQueryElem.attr("name")
                        );
                    }
                }
            }
