...
export class FormBuilder {
    ...
    loadTemplates(callback) {
        $.get(MapSVG.urls.root + "dist/" + this.template + ".html", (data) => {
            $(data).appendTo("body");
    ...
    init() {
        ...
        if (this.editMode) {
            ...
            this.formEditor = $(this.view).find("#mapsvg-data-form-edit")[0];
        }
        else {
    ...
    redraw() {
        ...
        if (_this.filtersMode && _this.searchButton) {
            _this.elements.buttons.searchButton = $('<div class="form-group mapsvg-filters-reset-container" id="mapsvg-search-container"><button class="btn btn-outline-secondary mapsvg-filters-reset">' +
